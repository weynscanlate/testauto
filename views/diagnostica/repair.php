<?php
$title = "Диагностика";
require_once($_SERVER['DOCUMENT_ROOT'] . "/assets/components/header.php");
?>
<div class="container">
    <div class="repair block_mt">
        <div class="repair_block_cont">
            <h2 class="h2_title">Ремонт</h2>
            <p class="repair_text">
                Ремонт модельного ряда вседорожников Toyota, в котором воплотились повышенная выносливость и проходимость, функциональность и комфортабельность.
            </p>
            <div class="repair_image">
                <img src="<?php $_SERVER['DOCUMENT_ROOT']; ?>/assets/img/car.png" alt="">
            </div>
        </div>
        <div class="repair_block_cont">
            <h2 class="h2_title">Товарные группы запчастей</h2>
            <div class="repair_block_items">
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        ХОДОВАЯ
                        ЧАСТЬ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        ЭЛЕМЕНТЫ
                        ПОДВЕСКИ
                    </h3>

                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        КОМПЛЕКТУЮЩИЕ
                        ДВИГАТЕЛЯ
                    </h3>

                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        РУЛЕВОЕ
                        УПРАВЛЕНИЕ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        МЕХАНИЗМЫ ГАЗОРАСПРЕДЕЛЕНИЯ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        НАВЕСНОЕ ОБОРУДОВАНИЕ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        ЭЛЕКТРООБОРУДОВАНИЕ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        ТОРМОЗНАЯ СИСТЕМА
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        РЕЗИНОТЕХНИЧЕСКИЕ ИЗДЕЛИЯ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        ВЫХЛОПНАЯ СИСТЕМА
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        АККУМУЛЯТОРЫ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        АВТОАКСЕССУАРЫ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        СИСТЕМА ЗАЖИГАНИЯ
                    </h3>
                </div>
                <div class="repair_block_item">
                    <h3 class="repair_h3_title">
                        СИГНАЛИЗАЦИЯ И АВТОЗВУК
                    </h3>
                </div>
            </div>
        </div>
    </div>

</div>
<?php require_once($_SERVER['DOCUMENT_ROOT'] . "/assets/components/footer.php"); ?>